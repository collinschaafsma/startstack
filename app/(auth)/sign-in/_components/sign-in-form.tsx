...
export default function SignInForm({
  ...
  const [optimisticSignIn, addOptimisticSignIn] = useOptimistic(
    "",
    (_state, newSignIn: string) => newSignIn
  )
  ...
  const [_state, formAction, isPending] = useActionState(
    async (_prevState: unknown, formData: FormData) => {
      ...
      try {
        await signInAction({ email, redirectTo, provider })
        return { success: true }
      } catch (error) {
        setHasError(true)
  ...
  return (
    <>
      {!optimisticSignIn && !hasError && !keepFormHidden ? (
        <form className={"space-y-4"} action={formAction}>
          {enabledProviders.includes("google") && (
            <Button
              ...
              disabled={isPending}
              className="flex w-full items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-xs hover:bg-gray-50 focus:outline-hidden focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
              name="provider"
              value="google"
            >
              {isPendingGoogle ? (
